...
// ChangeStar ViT-B is ~377MB; loading it under parallel Vitest OOMs GitHub runners.
const runHeavyTests =
  !process.env.CI || process.env.GEOAI_RUN_HEAVY_TESTS === "1";
...
describe("building-footprint-segmentation ChangeStar model routing", () => {
  ...
  it.skipIf(!runHeavyTests)(
    "routes changestar modelId to ChangeStarBuildingSegmentation",
    async () => {
  ...
  it.skipIf(!runHeavyTests)(
    "defaults to ChangeStar when modelId is omitted",
    async () => {
  ...
  it.skipIf(!runHeavyTests)(
    "routes explicit footprint modelId to BuildingFootPrintSegmentation",
    async () => {
      const instance = await geoai.pipeline(
        [
          {
  ...
  it.skipIf(!runHeavyTests)(
    "reuses the same ChangeStar instance for the same model",
    async () => {
  ...
  it.skipIf(!runHeavyTests)(
    "creates new instances for different providers",
    async () => {
      const instance1 = await geoai.pipeline([changestarTask], esriParams);
...
describe.skipIf(!runHeavyTests)(
  "test ChangeStar via building-footprint-segmentation",
  () => {
...
describe.skipIf(!runHeavyTests)(
  "test ChangeStar building footprint with geobase",
  () => {
